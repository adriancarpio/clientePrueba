		...
		function k() {
			if (V !== !1) {
				...
				u ? M || (b += y.outerWidth() - N) : M || (c += y.outerHeight() - N), V === !0 ? A.css({
					top : parseInt(c), left: parseInt(b)
	...
	var h = function() {
			...
			var c = this,
				...
				i = function(b, f) {
					...
					h || (c.settings.previewWidth || c.settings.previewHeight ? h = {
						previewWidth : c.settings.previewWidth, previewHeight: c.settings.previewHeight
		...
		k = function(b, c, d) {
			...
			return j ? (/^data\:image\/(png|jpe?g|gif);base64,[0-9A-Za-z\+\/\=]+$/.test(j) || (j = null), j ? {
				src : j, previewWidth : i, previewHeight: h,
				width: e,
				height: f
			}: null): null
		},
		...
		o = function(b, c) {
			...
			for (var j = [], k = {}, l = 0; l < b.length; l++) {
				var o = b[l],
					p = f ? o.name : o;
				if (!d || d.test(p)) if (e && e.test(p))"ext" in k || (k.ext = []), k.ext.push(p);
				else {
					var q;
					if (f) {
